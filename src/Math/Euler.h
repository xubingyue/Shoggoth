...
public:
    ...
    Euler &setDirection(const cinder::Vec3d &v, bool setYaw = true, bool setPitch = true)
    {
        ...
        if(setPitch)
            mPitch = sin(d.y);
        if(setYaw)
            mYaw = atan2(-d.x, -d.z);//+shmath::PI/2.0;
    ...
    inline void normalise(bool normYaw = true, bool normPitch = true, bool normRoll = true)
    {
        ...
        if(normPitch)
        {
            ...
            if(pitch < -shmath::PI)
            {
                pitch = fmod(pitch, shmath::TWO_PI);
                if(pitch < -shmath::PI)
                {
                    pitch += shmath::TWO_PI;
                }
